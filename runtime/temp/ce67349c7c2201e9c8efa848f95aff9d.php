<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:38:"./template/pc/rainbow/index/index.html";i:1510019854;s:40:"./template/pc/rainbow/public/header.html";i:1508395266;s:40:"./template/pc/rainbow/public/footer.html";i:1508381260;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
						...
						<div class="item">
							...
							<div class="cata-nav-layer">
								...
								<div class="advertisement_down">
									<?php $pid =10+$kr;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1536588000 and end_time > 1536588000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("5")->select();
...
<div id="myCarousel" class="carousel clearfix">
	<ul class="carousel-inner">
        <?php $pid =2;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1536588000 and end_time > 1536588000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("5")->select();
if(is_array($ad_position) && !in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v1):       
	...
	<?php $pid =52;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1536588000 and end_time > 1536588000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("2")->select();
if(is_array($ad_position) && !in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
<div class="adv3 w1224 clearfix">
	...
	<a  >
		<img   src="/public/images/4012.png" />
	</a>
</div>
...
<div class="floor floor<?php echo $k+1; ?> w1224">
	...
	<div class="floor-main">
		<div class="floor-brand">
			<?php $pid =10+$k;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1536588000 and end_time > 1536588000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(is_array($ad_position) && !in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$vl):       
    ...
    ?>
			...
			<?php endforeach; $pid =20+$k;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1536588000 and end_time > 1536588000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(is_array($ad_position) && !in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
		...
		<div class="floor-recommend">
			...
			<div class="floor-recommend-wrap">
				<div class="floor-recommend-list">
					<?php if(is_array($v[hot_goods]) || $v[hot_goods] instanceof \think\Collection || $v[hot_goods] instanceof \think\Paginator): if( count($v[hot_goods])==0 ) : echo "" ;else: foreach($v[hot_goods] as $gk=>$g): ?>
					<a class="floor-recommend-item" href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$g[goods_id])); ?>">
						<div class="floor-recommend-pic">
							<img class="w-100" src="<?php echo goods_thum_images($g[goods_id],200,200); ?>" alt="" />
						</div>
						<div class="floor-recommend-cont">
							<div class="recommend-goods-name ellipsis-1"><?php echo getSubstr($g[goods_name],0,15); ?></div>
							<div class="recommend-goods-des ellipsis-1"><?php echo $g[goods_remark]; ?></div>
							<div class="recommend-goods-price  recommend-goods-des">￥ <?php echo $g[shop_price]; ?></div>
						</div>
					</a>
					<?php endforeach; endif; else: echo "" ;endif; ?>
				</div>
