...
use think\Verify;
use think\Db;
use  app\home\model\User as  uModel ;
...
//            $this->verifyHandle('user_reg');
    ...
    public function edit_address(){
        header("Content-type:text/html;charset=utf-8");
        $id = I('get.id/d');
        $address = M('user_address')->where(array('address_id'=>$id,'user_id'=> $this->user_id,'is_delete'=>1))->find();
        if(IS_POST){
            $logic = new UsersLogic();
    ...
    public function set_default(){
        $id = I('get.id/d');
        M('user_address')->where(array('user_id'=>$this->user_id, 'is_delete'=>1))->save(array('is_default'=>0));
        $row = M('user_address')->where(array('user_id'=>$this->user_id,'address_id'=>$id , 'is_delete'=>1))->save(array('is_default'=>1));
        if(!$row)
            $this->error('操作失败');
        $this->success("操作成功");
    }
    ...
    public function del_address(){
        ...
        $row = M('user_address')->where(array('user_id'=>$this->user_id,'address_id'=>$id ,'is_delete'=>1))->save(array('is_delete'=>0)) ;
        // 如果删除的是默认收货地址 则要把第一个地址设置为默认收货地址
        if($address['is_default'] == 1)
        {
            $address2 = M('user_address')->where("user_id", $this->user_id)->where('is_delete', 1)->find();
            $address2 && M('user_address')->where("address_id", $address2['address_id'])->where('is_delete',1)->save(array('is_default'=>1));
        }
        if(!$row)
    ...
    */
    ...
    public function del_goods_collect(){
        ...
        if(!$id)
            $this->error("缺少ID参数");
        $row = M('goods_collect')->where(array('collect_id'=>$id,'user_id'=>$this->user_id,'is_delete'=>1))->save(['is_delete'=>0]);
        if(!$row)
            $this->error("删除失败");
        $this->success('删除成功');
    }
...
//            'is_agent'=>'1'
    ...
    public  function   Check(){
        ...
        $user_id = $this->user_id  ;
        //获取到订单编码
        $good_id =$_GET['goods_id'] ;   //  input('get.')
        $cer = M('goods_certificate')->where('goods_id','=',$good_id)->find();
        $info = [] ;
        $this->assign('good_id', $good_id) ;
        $this->assign('info', $info) ;
        $this->assign('cer', $cer) ;
//        $this->redirect('/home/_user/check') ;
        return  $this->fetch() ;
    }
...
//                //上传图片成功，返回到上一个页面
    ...
    {
        return $this->fetch('_user/message_notice');
    }
    /**
     * ajax用户消息通知请求
     * @author dyr
    ...
    public function ajax_message_notice()
    {
        ...
        return $this->fetch('_user/ajax_message_notice');
    }


    public  function  actionHandle(){
        $good_id =  request()->post('id') ;  //商品ID
        $tmp['good_id'] = $good_id;
        ...
        $license =  request()->post('license') ; //驾驶证
        if($license != NULL)
            $tmp['license'] = $license;

        $credit =  request()->post('credit') ; //银行卡
        if($credit != NULL)
            $tmp['credit'] = $credit;

        $security =  request()->post('security') ; //社保卡 营业执照
        if($security != NULL)
            $tmp['security'] = $security;

        $bankflow =  request()->post('bankflow') ; //银行流水
        if($bankflow != NULL)
            $tmp['bankflow'] = $bankflow;

        $ownership =  request()->post('ownership') ; //房产证
        if($ownership != NULL)
            $tmp['ownership'] = $ownership;

        $commencial =  request()->post('commencial') ; //商业保险单
        if($commencial != NULL)
            $tmp['commencial'] = $commencial;

        $imgdata =  M('user_good_image')->where(array('user_id'=> $tmp['user_id'],'good_id'=>$tmp['good_id']))->find() ;
        if(empty($imgdata)){
            $tmp['create_time'] = time();
            $res =   M('user_good_image')->save($tmp) ;
        }else{
            $res =  M('user_good_image')->where(array('user_id'=> $tmp['user_id'],'good_id'=>$tmp['good_id']))->save($tmp);
        }
