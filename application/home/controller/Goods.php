...
class Goods extends Base {
    ...
    public function goodsInfo(){
        ...
        $inst = M('task')->where('goods_id','=',$goods_id)->find();
...
//        $goods_attribute = M('GoodsAttribute')->getField('attr_id,attr_name'); // 查询属性
        $goods_attr_list = M('GoodsAttr a')
            ->join('goodsAttribute b','a.attr_id = b.attr_id',LEFT)
            ->where("goods_id", $goods_id)->select(); // 查询商品属性表
//        var_dump($goods_id);
        ...
        if($this->user_id > 0){
            ...
            //已经登录
            $udata =  M('user_good_image')->where(array('user_id'=> $this->user_id,'good_id'=>$goods_id,'isdelete'=>0))->find();
            if(!empty($udata)){
                if ($udata['view'] == '0' ){
                    //还没有认证的界面
                    $this->assign('statu', 1 );
                    ...
                    //直接跳转到下订单页面
                    $this->assign('statu', 2 );
                }elseif ($udata['view'] == '2' || $udata['view'] ==3){
                    //审核未通过 请重新提交
                    $this->assign('statu', 3 );
                }
            }else{
        ...
        $this->assign('inst', $inst);//任务数
        $this->assign('var', $var);//查询上级
...
//        $this->assign('goods_attribute',$goods_attribute);//属性值
    ...
    public function goodsList(){
        ...
        $filter_goods_id = M('goods')->where(['is_on_sale'=>1,'cat_id'=>['in',implode(',', $cat_id_arr)]])->cache(true)->getField("goods_id",true);
        // 过滤帅选的结果集里面找商品
        ...
        {
            if($this->user_id > 0){
                $users=M('users')->where('user_id','=',$this->user_id)->find();
                if($users['inst_id'] == 2){
                    $goods_list = M('goods')->where("goods_id","in", implode(',', $filter_goods_id))->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();
                }else{
                    $goods_list = M('goods')->where("goods_id","in", implode(',', $filter_goods_id))->where('inst_id','=',1)->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();
                }
            }else{
                $goods_list = M('goods')->where("goods_id","in", implode(',', $filter_goods_id))->where('inst_id','=',1)->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();
            }
