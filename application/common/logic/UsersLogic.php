...
class UsersLogic extends Model
{
    ...
    * */
     ...
     * @return array
     */
    public function reg($nickname,$mobile,$password,$password2, $push_id=0,$invite=array(), $id_card='',$type){
...
//        }
        ...
        if(!empty($invite)){
            ...
            $invi = M('users')->where('user_id','=',$invite['user_id'])->find();
            $map['inst_id'] = $invi['inst_id'];
        }else{
        ...
        return array('status'=>1,'msg'=>'注册成功','result'=>$user,'type'=>$type);
    }
    ...
    public function get_goods_collect($user_id){
        ...
        $result = M('goods_collect')->alias('c')
            ...
            ->join('goods g','g.goods_id = c.goods_id','INNER')
            ->where(array('c.user_id'=>$user_id, 'c.is_delete'=>1))
            ->limit($page->firstRow,$page->listRows)
            ->select();
        $return['status'] = 1;
        $return['msg'] = '获取成功';
