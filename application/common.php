...
function get_user_address_list($user_id){
    $lists = M('user_address')->where(array('user_id'=>$user_id,'is_delete'=>1))->select();
    return $lists;
}
...
function confirm_order($id,$user_id = 0){
    ...
    $task=M('order_goods a')->join('task b','a.goods_id = b.goods_id')->find();
    $num = $task['number'];
    ...
    // 查询用户信息
    $number = M('users ')->where('user_id', $user_id)->find();
    //根据用户信息查询上级
    $num = M('users ')->where('user_id', $number['uid'])->find();
    //用户下单成功之后task_user表里的完成数量+1
    if ($num != null) {
        rebate($id,$user_id);
        $number_order = M('user_task')->where('user_id','=',$num['user_id'])->find();
 ...
 * @param $uid/上级用户id
 */
function rebate($order,$uid){
    $users = M('users')->field('user_id,uid,nickname')->where('user_id','=',$uid)->find(); //查询用户信息
    $user_id = M('users')->field('user_id,distribut_money')->where('user_id','=',$users['uid'])->find(); //查询用户上级信息

    $id = M('order o')
        ->join('goods g','o.order_id = g.goods_id')
        ->where('o.order_id','=',$order)
        ->find(); //查询用户商品订单信息
    $task = M('task')->field('number')->where('goods_id','=',$id['goods_id'])->find();//查询商品任务数
    $distribut = round(($id['goods_price'] * 0.7) / $task['number'], 2);//计算佣金
    $money =$user_id['distribut_money']+$distribut;
    $update_data = array(
        'distribut_money'=> $money ,
    );
    M('users')->where('user_id','=',$user_id['user_id'])->save($update_data);
