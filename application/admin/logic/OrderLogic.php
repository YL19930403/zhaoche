...
class OrderLogic extends Model
{
    ...
    public function getGoodsAmount($order_id){
        $sql = "SELECT SUM(goods_num * goods_price) AS goods_amount FROM __PREFIX__order_goods WHERE order_id = {$order_id} and is_delete = 1 ";
        $res = DB::query($sql);
...
//        	$btn['delivery_confirm'] = '确认提车';
    ...
    public function orderProcessHandle($order_id,$act,$ext=array()){
    	...
    	switch ($act){
    		case 'pay': //付款
               	$order_sn = M('order')->where(array('order_id'=>$order_id, 'is_delete'=>1))->getField("order_sn");
    	...
    	return M('order')->where(array('order_id'=>$order_id, 'is_delete'=>1))->save($updata);//改变订单状态
    }
    ...
    public function deliveryHandle($data){
		...
		M('order')->where(array('order_id'=>$data['order_id'], 'is_delete'=>1))->save($updata);//改变订单状态
    ...
    function delOrder($order_id){
    	...
    	$del_order = M('order')->where(array('order_id'=>$order_id,'is_delete'=>1))->save(array('is_delete'=>0));
