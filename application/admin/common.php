...
function updateRefundGoods($rec_id,$refund_type=0){
	...
	//使用积分或者余额抵扣部分原路退还
	if(($return_goods['refund_deposit']>0 || $return_goods['refund_integral']>0) && $refund_type==0){
...
function updateRefundOrder($order_id){
	$order = M('order')->where(['order_id'=>$order_id,'pay_status'=>3,'pay_status'=>1])->find(); //只有已付款，已取消的才能有
    ...
    M('order')->where(array('order_id'=>$order_id))->save(array('pay_status'=>3));
    $expense_data = array('money'=>$order['order_amount'],'log_type_id'=>$order['order_id'],'type'=>2,'user_id'=>$order['user_id']);
