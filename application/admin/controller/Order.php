...
class Order extends Base {
    ...
    public function ajaxindex(){
        ...
        I('order_status') != '' ? $condition['d.order_status'] = I('order_status') : false;
        I('pay_status') != '' ? $condition['d.pay_status'] = I('pay_status') : false;
        I('pay_code') != '' ? $condition['d.pay_code'] = I('pay_code') : false;
        I('shipping_status') != '' ? $condition['shipping_status'] = I('shipping_status') : false;
        I('user_id') ? $condition['user_id'] = trim(I('user_id')) : false;
        $sort_order = I('d.order_by','DESC').' '.I('sort');
        ...
        if($admin_info['admin_id'] == 1) {
            ...
            $count =M('inst a')
                ->join('users c','a.inst_id = c.inst_id',LEFT)
                ->join('order d','c.user_id = d.user_id')
                ->where($condition)
                ->count();
            $Page  = new AjaxPage($count,20);
            //  搜索条件下 分页赋值
...
//        var_dump($orderList);
    ...
    */
    public function ajaxdelivery(){
        ...
        I('consignee') ? $condition['d.consignee'] = trim(I('consignee')) : false;
        I('order_sn') != '' ? $condition['d.order_sn'] = trim(I('order_sn')) : false;
        $shipping_status = I('shipping_status');
        $condition['d.shipping_status'] = empty($shipping_status) ? array('neq',1) : $shipping_status;
        $condition['d.order_status'] = array('in','1,2,4');
        $condition['d.is_delete'] = 1 ;
        if($admin_info['admin_id'] ==1){
            $count = M('order')->where($condition)->count();
            ...
            $count =M('inst a')
                ->join('users c','a.inst_id = c.inst_id',LEFT)
                ->join('order d','c.user_id = d.user_id',LEFT)
                ->where($condition)
                ->count();
            $Page  = new AjaxPage($count,10);
            //搜索条件下 分页赋值
    ...
    public function refund_order_list(){
        ...
        if($admin_info['admin_id']==1){
            ...
            $condition['c.plat_admin_id'] =$admin_info['admin_id'];
            $condition['c.open_status'] = 1;
            $count =M('users a')
                ->join('inst c','c.inst_id = a.inst_id',LEFT)
                ->join('user_good_image e', 'e.user_id = a.user_id',RIGHT)
                ->join('goods f','e.good_id = f.goods_id',RIGHT)
                ->where($condition)
                ->count();
            $Page  = new Page($count,10);
            //搜索条件下 分页赋值
    ...
    public function refund_order(){
        $data = I('post.');
        if($data){
          M('user_good_image')->where(array('user_id'=>$data['user_id'],'good_id'=>$data['good_id']))->save(array('view'=>$data['view']));
        }
        $user = M('users')->where('user_id','=',$data['user_id'])->find();
        if($data['view'] == 1){
            $msg  = '尊敬的'.$user['nickname'].'用户，您提交的资料已审核通过，请到门店进行后续提车操作';
            $goods = M('goods')->where('goods_id','=',$data['good_id'])->select();
            foreach ($goods as $val ){
                $arr=[
                    ...
                    'member_goods_price' => $val['shop_price'],
                    'goods_num'      => 1,
                    'add_time'       => time(),
                    'prom_id'        => 0,
        ...
        //用户下单, 发送短信给商家
        sendSms($user['mobile'],$msg,'',$scene=3, 'true');
...
//    }
    ...
    public function split_order(){
        ...
        if(IS_POST){
            ...
            M('order')->where(array('order_id'=>$order_id, 'is_delete'=>1))->save($res);
            //################################原单处理结束
    ...
    public function editprice($order_id){
        ...
        if(IS_POST){
            ...
            $row = M('order')->where(array('order_id'=>$order_id,'is_delete'=>1))->save($update);
            if(!$row){
    ...
    public function pay_cancel($order_id){
        if(I('remark')){
            ...
            $order = M('order')->where(array('order_id'=>$order_id, 'is_delete'=>1))->find();
            $this->assign('order',$order);
            return $this->fetch();
        }
    }
        ...
        if(IS_POST)
        {
            ...
            if($result && $data['status']==1)
            {
                ...
                if($return_goods['type'] < 2){
                    //$orderLogic->closeOrderByReturn($return_goods['order_id']); //当订单里商品都退货审核通过，将订单状态改成关闭
                    $orderLogic->disposereRurnOrderCoupon($return_goods); // 是退货可能要处理优惠券
                }
            }
            $log = $orderLogic->orderActionLog($return_goods['order_id'],'退换货',$note);
            $this->success('修改成功!');
            exit;
        }
    ...
    public function refund_balance(){
        ...
        $data['refund_money'] = $data['refund_money']>0 ? $data['refund_money'] : 0;
        $data['refund_deposit'] = $data['refund_deposit']>0 ? $data['refund_deposit'] : 0;
        $data['refund_integral'] = $data['refund_integral']>0 ? $data['refund_integral'] : 0;
        $refund_money = $data['refund_deposit'] + $data['refund_money'];
        if(accountLog($return_goods['user_id'],$refund_money,$data['refund_integral'],'用户申请商品退款',0,$return_goods['order_id'],$return_goods['order_sn'])){
            M('return_goods')->where(array('rec_id'=>$data['rec_id']))->save($data);
            updateRefundGoods($data['rec_id'],1);
            $this->ajaxReturn(['status'=>1,'msg'=>"操作成功",'url'=>U("Admin/Order/return_list")]);
        }else{
            $this->ajaxReturn(['status'=>0,'msg'=>"退款操作失败"]);
        }
    }
    ...
    public function ajax_return_list(){
        ...
        if($admin_info['admin_id']==1){
            ...
            $where['c.plat_admin_id'] =$admin_info['admin_id'];
            $where['c.open_status'] = 1;
            $count =M('users a')
                ->join('inst c','c.inst_id = a.inst_id',LEFT)
                ->join('user_task d','a.user_id = d.user_id',RIGHT)
                ->where($where)
                ->count();
            $Page  = new AjaxPage($count,13);
            $show = $Page->show();
    ...
    public function ajaxOrderNotice(){
        $order_amount = M('order')->where("order_status=0  and is_delete = 1 and (pay_status=1 or pay_code='cod')")->count();
        echo $order_amount;
    }
