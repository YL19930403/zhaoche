...
class Topic extends Base {
    ...
    public function agreeAdd(){
        if(IS_POST){
            ...
            $agree=D('agreement')->add($data);
            if($agree){
                $this->success("操作成功",U('Admin/Topic/agreement'));
            }else{
                $this->error("操作失败",U('Admin/Topic/agreement'));
            }
        }
        return $this->fetch();
    }

    public function agreeEdit(){
        $id = $_GET['id'];
        $agree=D('agreement')->where('id','=',$id)->find();
        $this->assign('agree',$agree);
        if(IS_POST){
            $data=[
                'title'    =>$_POST['title'],
            ...
            $agree=D('agreement')->where('id','=',$id)->save($data);
            if($agree){
                $this->success("操作成功",U('Admin/Topic/agreement'));
            }else{
                $this->error("操作失败",U('Admin/Topic/agreement'));
