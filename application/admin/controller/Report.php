...
class Report extends Base{
    ...
    public function organization(){
        ...
        $show = $Page->show();
        $sql = M('inst')->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function referrer(){
        ...
        if($admin_info['admin_id'] ==1){
            $count = M('inst a')
                    ->join('users b','a.inst_id = b.inst_id')
                    ->where('b.is_agent > 0')
                    ->count();
            $Page = new Page($count,10);
            $show =  $Page->show();
            $sql = M('inst a')
                ...
                ->order('b.reg_time desc')
                ->select();
        }else{
            $count = M('inst a')
                ->join('users b','a.inst_id = b.inst_id')
                ->where(array('plat_admin_id'=>$admin_info['admin_id'],'b.is_agent'=>array('gt',0)))
                ->count();
            $Page = new Page($count,10);
            $show =  $Page->show();
            $sql = M('inst a')
...
//    }
    ...
    public  function refEdit(){
        ...
        $sql = M('inst')->where('open_status','=',1)->select();
        $ref=M('inst a')
            ->join('users b','b.inst_id = a.inst_id')
            ->where('b.user_id','=',$_GET['user_id'])
            ->find();
        if(IS_POST){
            $arr=[
                'inst_id' => $_POST['inst_id']
            ];
            $a=M('users')->where('user_id','=',$_GET['user_id'])->save($arr);
            if($a){
                $this->success('操作成功',U('Admin/Report/referrer'));
            }else{
                $this->success('操作失败',U('Admin/Report/referrer'));
