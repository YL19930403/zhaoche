...
class Goods extends Base {
    ...
    public function categoryList(){
        ...
        $inst = M('inst')->where('plat_admin_id','=',$admin_info['admin_id'])->find();
    ...
    public function addEditCategory(){
        $admin_info = getAdminInfo(session('admin_id'));
        $inst = M('inst')->where('plat_admin_id','=',$admin_info['admin_id'])->find();
        $name =M('inst a')->join('goods_category b','a.inst_id = b.inst_id')->where('b.id','=',$_GET['id'])->find();
        $sql = M('inst')->select();
        $this->assign('sql',$sql);
    ...
    public function goodsList(){
        ...
        I('brand_id') && $where = "$where and a.brand_id = ".I('brand_id') ;
        (I('is_on_sale') !== '') && $where = "$where and a.is_on_sale = ".I('is_on_sale') ;
        $cat_id = I('cat_id');
        ...
        if($admin_info['admin_id'] == 1){
            $count = M('Goods a')->where($where)->count();
...
//            $order_str = "`{$_POST['orderby1']}` {$_POST['orderby2']}";
            ...
            $where .= 'and b.plat_admin_id = '.$admin_info['admin_id'] ;
            $count = M('Goods a')
                ->join('inst b','a.inst_id = b.inst_id')
                ->where($where)
                ->count();
            $Page  = new Page($count,10);
            $show = $Page->show();
//            $order_str = "`{$_POST['orderby1']}` {$_POST['orderby2']}";
        ...
        $categoryList = M('goods_category')->where('parent_id',0)->select();
...
//        var_dump($a);
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
          ...
          $GoodsLogic->saveGoodsAttr($goods_id, I('goods_type')); // 处理商品 属性
            $return_arr = array(
                'status' => 1,
                'msg' => '操作成功',
                'data' => array('url' =>  'javascript:history.back()'),
            );
            $this->ajaxReturn($return_arr);
        }
        $type = M('type')->where('is_delete','=',1)->select();
    ...
    public function goodsTypeList(){
        ...
        $show  = $Page->show();
        $goodsTypeList = $model->where('is_delete',1)->order("id desc")->limit($Page->firstRow.','.$Page->listRows)->select();
        $this->assign('inst',$inst);
    ...
    public function editType(){
        ...
        $id = $_GET['type_id'];
        $type=M('type')->where('type_id','=',$id)->find();
        if(IS_POST){
            $data=[
                'type_name' => $_POST['name'],
                'inst_id'   => $_POST['inst_id']
            ];
            $save =M('type')->where('type_id','=',$id)->save($data);
            if($save){
                $this->success("操作成功!!!", U('Admin/Goods/goodsType'));
            }else{
                $this->error("操作失败!!!", U('Admin/Goods/goodsType'));
            }
        }
        $this->assign('sql',$sql);
        $this->assign('name',$name);
    ...
    public function delGoodsType()
    {
        ...
        M('GoodsType')->where("id = {$id}")->save($data);
        $this->success("操作成功!!!",U('Admin/Goods/goodsTypeList'));
    }
...
//        $model->where('id ='.$_GET['id'])->delete();
        $a=$model->where('id ='.$_GET['id'])->update(['is_delete'=>0]);
        var_dump($a);die;
        $return_arr = array('status' => 1,'msg' => '操作成功','data'  =>'',);   //$return_arr = array('status' => -1,'msg' => '删除失败','data'  =>'',);
        $this->ajaxReturn($return_arr);
    }
    ...
    public function ajaxSpecList(){
        ...
        $where = ' 1 = 1 '; // 搜索条件
        I('type_id')   && $where = "$where and type_id = ".I('type_id') ;
//        $where = "$where and is_delete = 1 ";  // 关键词搜索
        $model = D('spec');
...
//        var_dump($specList);
    ...
    public  function addEditSpec(){
        ...
        $admin_info = getAdminInfo(session('admin_id'));
        $inst = M('inst')->where('plat_admin_id','=',$admin_info['admin_id'])->find();
        $name =M('inst a')->join('spec b','a.inst_id = b.inst_id')->where('b.id','=',$_GET['id'])->find();
        $sql = M('inst')->select();
        $this->assign('sql',$sql);
        $this->assign('name',$name);
        $this->assign('inst',$inst);
    ...
    public function ajaxGetSpecSelect(){
        ...
        //$_GET['spec_type'] =  13;
        $arr=[
            'type_id'     => I('get.spec_type/d' ),
            'is_delete'   => 1
        ];
        $specList = M('Spec')->where($arr)->order('`order` desc')->select();
        foreach($specList as $k => $v)
    ...
    public function payment(){
        $payment = M('goods_payment')->select();
    ...
    public function addEditpayment(){
        ...
        $payment = M('goods_payment')->where('pay_id','=',$_GET['pay_id'])->find();
        if(IS_POST){
            $data['radion'] = $_POST['name'];
            if($_GET['pay_id'] != null){
                $pay=M('goods_payment')->where('pay_id','=',$_GET['pay_id'])->save($data);
            }else{
                $data['create_time'] = time();
                $pay=M('goods_payment')->add($data);
            }
            if($pay){
                $this->success("操作成功!!!",U('Admin/Goods/payment'));
            }else{
                $this->error("操作失败!!!",U('Admin/Goods/payment'));
            }
        }
        $this->assign('payment',$payment);
        return $this->fetch();
    ...
    public function ajaxPaymentInfo(){
        ...
        if($key_word)
        {
            $where = "$where and (b.goods_name like '%$key_word%' or b.goods_sn like '%$key_word%')" ;
        }

        $where = "$where and(a.is_delete = 1)";
        $count = M('goods_payment_info a')
            ->join('goods b','a.goods_id = b.goods_id')
            ->join('goods_payment c','a.pay_id = c.pay_id')
            ->where($where)
            ->count();
        ...
        $show = $Page->show();
        $payment_info = M('goods_payment_info a')
            ->join('goods b','a.goods_id = b.goods_id')
            ->join('goods_payment c','a.pay_id = c.pay_id')
            ->where($where)
            ->limit($Page->firstRow.','.$Page->listRows)
...
//        var_dump($payment_info) ; die ;
    ...
    public function addEditinfo(){
        ...
        $payment = M('goods_payment')->where('is_delete','=',1)->select();
        $paymentInfo = M('goods_payment_info')->where('id','=',$_GET['id'])->find();
        if(IS_POST){
            if($_POST['goods_id'] =='')
                $this->error('请选择商品');
            if($_POST['pay_id'] == '')
            ...
            if($_GET['id'] != null){
                $pay_info =M('goods_payment_info')->where('id','=',$_GET['id'])->save($data);
            }else{
                $pay = M('goods_payment_info')->where(array('goods_id'=>$_POST['goods_id'],'pay_id'=>$_POST['pay_id'],'is_delete'=>1))->count();
                if($pay >0)
                    $this->error('商品比例已存在');
                $data['create_time'] = time();
                $pay_info =M('goods_payment_info')->add($data);
            }
            if($pay_info>0){
                $this->success("操作成功!!!",U('Admin/Goods/payment_info'));
            }else{
                $this->error("操作失败!!!",U('Admin/Goods/payment_info'));
            }
        }
    ...
    public function delInfo(){
        ...
        $del =M('goods_payment_info')->where('id','=',$_GET['id'])->save($data);
        if($del)
        $this->success("删除操作成功!!!",U('Admin/Goods/payment_info'));
